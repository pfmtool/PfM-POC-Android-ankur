...
public class UserDatastore {
	...
	private static final Logger mLogger = Logger
			.getLogger(UserDatastore.class.getName());
	...
	public static boolean add(User contact) {
		...
		Entity entity = new Entity(User.USER_ENTITY_NAME, contact.mName,
				parentKey);
		entity.setProperty(User.FIELD_NAME_NAME, contact.mName);
		entity.setProperty(User.FIELD_NAME_PASS, contact.mPassword);
	...
	public static ArrayList<User> query(String name) {
		...
		if (name != null && !name.equals("")) {
			User contact = getContactByName(name, null);
			if (contact != null) {
	...
	public static User getContactByName(String name, Transaction txn) {
		Entity result = null;
		try {
			result = mDatastore.get(KeyFactory.createKey(getKey(),
